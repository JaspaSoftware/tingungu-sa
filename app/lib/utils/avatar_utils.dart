...
class AvatarUtils {
  ...
  static ImageProvider? getAvatarImageProvider(String? avatarUrl) {
    ...
    if (trimmed.startsWith('asset:') || trimmed.startsWith('assets/') || trimmed.startsWith('lib/assets/')) {
      String path = trimmed;
    ...
    try {
      final String base64Data = trimmed.contains(',') ? trimmed.split(',').last : trimmed;
  ...
  static ImageProvider getAvatarImageProviderOrDefault(String? avatarUrl, {String defaultAsset = 'lib/assets/images/avatar_cross_flame.png'}) {
  ...
  static Future<void> showAvatarUploadOptions(
      ...
      try {
        ...
        if (image != null) {
          ...
          if (context.mounted) {
            ...
            await saveAvatar(context, base64String, onAvatarUpdated: onAvatarUpdated);
          }
        }
      } catch (e) {
        if (context.mounted) {
    ...
    Future<void> showUrlInputDialog() async {
      final controller = TextEditingController(text: currentAvatar.startsWith('http') ? currentAvatar : '');
      final String? url = await showDialog<String>(
        context: context,
        builder: (ctx) => AlertDialog(
          title: const Text('Enter Image URL', style: TextStyle(color: Color(0xFF3B0D11))),
          content: TextField(
              ...
              children: [
                ...
                ListTile(
                  leading: Container(
                    ...
                    child: const Icon(Icons.photo_library, color: Color(0xFFFB8B24)),
                  ),
                ...
                ListTile(
                  leading: Container(
                    ...
                    child: const Icon(Icons.camera_alt, color: Color(0xFF3B0D11)),
                  ),
                ...
                ListTile(
                  ...
                  title: const Text('Image Web Link', style: TextStyle(fontWeight: FontWeight.w600)),
                ...
                if (currentAvatar.isNotEmpty) ...[
                  ListTile(
                    ...
                    title: const Text('Remove Current Avatar', style: TextStyle(color: Colors.red, fontWeight: FontWeight.w600)),
                    onTap: () async {
                      onAvatarUpdated?.call('');
                      Navigator.pop(ctx);
                      await saveAvatar(context, '', onAvatarUpdated: onAvatarUpdated);
                    },
                  ),
                ],
